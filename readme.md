...
The `class CFE<TokenId, ASTId, Source>` can be used to construct a Compiler Front End.

The `CFE class` contains the functionality to convert a source into tokens, then parse the tokens into an Abstract Syntax Tree (AST).

The `TokenId` type should an enumeration of token identifiers.

The `ASTId` type should be an enumeration of AST node identifiers.

Here is an example of a calculator CFE, taken from unit tests:
...
The calculator CFE class can be used like this:
...
The tokenizer and parser grammars can be defined in a class, as in the above example, or as global variables, depending on user choice.

Declaring everything in a class has the additional benefit of template parameters, for example parameterizing a CFE on the source type.

The default source type for `class CFE` is the `class SourceString<>`, allowing the output AST and errors to contain line and column information.
