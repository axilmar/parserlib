...
static void unitTest_EBNF_createCFE() {
    SourceString<> input =
        "%comment = '(*' (_ - '*)')* '*)';"
        "                                 "
        "%whitespace = '\\0' .. ' ';      "
    ...
    EBNF<>::createCFE(std::cout, "PARSERLIB_CFE_CALCULATOR_HPP", "parserlib::cfe", "CalculatorCFE", ast);
}
