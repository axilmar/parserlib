...
namespace parserlib {
    ...
    class logical_not_parse_node : public parse_node<ParseContext> {
    public:
        logical_not_parse_node(const parse_node_ptr<ParseContext>& parse_node)
    ...
    parse_node_ptr<ParseContext> operator !(const parse_node_ptr<ParseContext>& parse_node) {
        if (dynamic_cast<const logical_not_parse_node<ParseContext>*>(parse_node.get())) {
            return parse_node;
        }
        return std::make_shared<logical_not_parse_node<ParseContext>>(parse_node);
    }
