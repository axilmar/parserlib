...
namespace parserlib {
    ...
    public:
        ...
        bool valid() const {
            return m_parse_position.iterator != m_string.end();
        }

        template <class T> bool parse_symbol(const T& value) {
            if (valid() && Traits::to_lower(*m_parse_position.iterator) == Traits::to_lower(value)) {
        ...
        template <class T> bool parse_symbol_string(const T& string) {
            iterator_type this_it = m_parse_position.iterator;
    ...
    private:
        ...
        struct state {
            struct parse_position parse_position;
        };
        ...
        void increment_parse_position() {
            ++m_parse_position.iterator;
            m_parse_position.position.increment_column();
        }

        state get_state() const {
            return state{ m_parse_position };
        }
