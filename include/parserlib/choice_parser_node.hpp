...
namespace parserlib {
    ...
    auto operator | (const L& left, const R& right) {
        if constexpr (std::is_base_of_v<choice_parser_node_tag, L> && std::is_base_of_v<choice_parser_node_tag, R>) {
            return choice_parser_node(std::tuple_cat(left.parsers(), right.parsers()));
        }
        else if constexpr (std::is_base_of_v<choice_parser_node_tag, L>) {
            return choice_parser_node(std::tuple_cat(left.parsers(), std::make_tuple(parser(right))));
        }
        else if constexpr (std::is_base_of_v<choice_parser_node_tag, R>) {
