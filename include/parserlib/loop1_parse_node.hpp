...
namespace parserlib {
    ...
    class loop1_parse_node : public interface::parse_node {
    public:
        loop1_parse_node(const parserlib::parse_node& child) : m_child(child) {
        }
    ...
    inline parse_node parse_node::operator +() const {
        return interface::create_parse_node<loop1_parse_node>(*this);
    }
